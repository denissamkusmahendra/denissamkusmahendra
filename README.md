<div style={{ position: 'relative', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', width: '100%', height: '100%', overflow: 'hidden', borderRadius: 20, background: '#0B0F14', border: '1px solid #243447', fontFamily: 'Inter, sans-serif', padding: '34px 42px' }}>
  ...
  <svg width="860" height="300" style={{ position: 'absolute', inset: 0 }}>
    <defs>
      ...
      {/* Changed amberGlow to blueGlow */}
      <radialGradient id="blueGlow" cx="5%" cy="0%" r="72%">
        <stop offset="0%" stopColor="rgba(59,130,246,.13)" /> {/* Updated to blue RGB */}
        <stop offset="100%" stopColor="rgba(59,130,246,0)" />
      </radialGradient>
    </defs>
    <rect width="860" height="300" fill="url(#grid)" />
    <rect width="860" height="300" fill="url(#cyanGlow)" />
    <rect width="860" height="300" fill="url(#blueGlow)" /> {/* Updated fill reference */}
    <g id="orbit">
    ...
    {/* Changed stroke from amber to blue */}
    <circle cx="742" cy="151" r="53" fill="rgba(11,15,20,.64)" stroke="rgba(59,130,246,.34)" />
    ...
    {/* Changed graph point fill from amber to blue */}
    <circle cx="772" cy="130" r="4" fill="#3B82F6" />
  </svg>

  <div style={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
    {/* Changed label text color from amber to blue */}
    <span style={{ color: '#3B82F6', fontSize: 12, fontWeight: 800, fontFamily: 'monospace', letterSpacing: 2.2 }}>DENIS / PROFILE</span>
  ...
  <div style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 9 }}>
    {['SQL • BigQuery', 'Python', 'Power BI • Excel'].map(function(item, index) {
      {/* Changed the highlighted skill tag color from amber to blue */}
      return <span key={item} style={{ color: index === 2 ? '#3B82F6' : '#CBD5E1', border: '1px solid rgba(148,163,184,.22)', borderRadius: 7, background: 'rgba(15,23,42,.72)', padding: '7px 10px', fontSize: 12, fontWeight: 700, fontFamily: 'monospace', letterSpacing: .7 }}>{item}</span>;
    })}
  </div>
</div>
